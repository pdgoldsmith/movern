...
from movern.artifacts import ClassificationModel
from movern.evaluators.evaluator import Evaluator
from movern.evaluators.utils.fairlearn import setup_metric_frames
from movern.evaluators.utils.validation import check_data_for_nulls, check_existence
from movern.modules.metrics import process_metrics
from movern.utils.common import ValidationError
...
class Performance(Evaluator):
    ...
    ------------------
        **Required Artifacts**
        ...
        Generally artifacts are passed directly to :class:`movern.lens.Lens`, which
        ...
        - model: :class:`movern.artifacts.Model`
        - assessment_data: :class:`movern.artifacts.TabularData`

    Parameters
    ----------
    metrics : List-like
        list of metric names as strings or list of Metric objects (movern.modules.metrics.Metric).
        Metric strings should in list returned by movern.modules.metric_utils.list_metrics().
    ...
    def update_metrics(self, metrics, replace=True):
        ...
        ----------
        metrics : List-like
            list of metric names as string or list of Metrics (movern.metrics.Metric).
            Metric strings should in list returned by movern.modules.list_metrics.
            Note for performance parity metrics like
            "false negative rate parity" just list "false negative rate". Parity metrics
            are calculated automatically if the performance metric is supplied
        """
