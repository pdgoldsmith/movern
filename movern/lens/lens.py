...
from movern.artifacts import Data, Model
from movern.evaluators.evaluator import Evaluator
from movern.lens.pipeline_creator import PipelineCreator
from movern.utils import (
    ValidationError,
    check_subset,
    flatten_list,
    global_logger,
)
from movern.lens.lens_validation import check_model_data_consistency
...
class Lens:
    ...
    def _validate(self):
        ...
        if not (isinstance(self.assessment_data, Data) or self.assessment_data is None):
            raise ValidationError(
                "Assessment data should inherit from movern.artifacts.Data"
            )
